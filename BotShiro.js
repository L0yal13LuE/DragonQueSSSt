...
const handleBagCommand = async (message) => {
    ...
    try {
        ...
        const itemList = (userItems && userItems.length > 0)
            ? userItems.map(item => `${item.itememoji} ${item.itemname}: ${item.itemamount}`).join('\n')
...
client.once('ready', async () => {
    ...
    // Initialize Cooldowns from Database
    // if (supabase) {
    //     try {
    //         const { data: users, error } = await supabase.from('users').select('id, last_online_timestamp');
    //         if (error) throw error;
    //         let count = 0;
    //         for (const user of users) {
    //             if (user.last_online_timestamp) {
    //                 userCooldowns.set(user.id, new Date(user.last_online_timestamp).getTime());
    //                 count++;
    //             }
    //         }
    //         console.log(`Initialized cooldowns for ${count} users from database.`);
    //     } catch (error) {
    //         console.error('Error fetching initial cooldowns:', error.message);
    //     }
    // }
