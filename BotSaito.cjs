...
async function processGifQueue() {
	...
	try {
		...
		try {
			...
			const imageOutputFile = await useLocalPython(url, startTime, endTime, outputFile);
...
client.on("messageCreate", async (message) => {
	...
	if (gifCommandMatch) {
		...
		const startTime = parseTime(match[2]) || '00:00';
		const endTime = parseTime(match[3]) || '00:00';
