 ...
 */
const fetchCraftCommand = async (command) => {
 ...
 */
const fetchCraftMaterials = async (craftId) => {
    if (!supabase) { message.reply("ฐานข้อมูลมีปัญหา โปรดติดต่อผู้ดูแล"); return; }
 ...
 */
const fetchUserItemsByMaterialIds = async (userId, materialNames) => {
    if (!supabase) { message.reply("ฐานข้อมูลมีปัญหา โปรดติดต่อผู้ดูแล"); return; }
 ...
 */
const fetchCraftItemsRequired = async (message, craftCommand) => {
    if (!supabase) { message.reply("ฐานข้อมูลมีปัญหา โปรดติดต่อผู้ดูแล"); return; }

    // fetch requred materials
    const materials = await fetchCraftMaterials(craftCommand.id);

    // fetch user materials
    const materialNames = materials.map(item => item.materials.name);
    const userMaterials = await fetchUserItemsByMaterialIds(message.author.id, materialNames);
    return {
        materials,
        userMaterials,
    }
 ...
 */
const fetchNewCraftItems = async (material_id) => {
    if (!supabase) { message.reply("ฐานข้อมูลมีปัญหา โปรดติดต่อผู้ดูแล"); return; }
 ...
 */
const handleCraftCommand = async (command, message, client) => {
    if (!supabase) { message.reply("ฐานข้อมูลมีปัญหา โปรดติดต่อผู้ดูแล"); return; }
    ...
    const craftCommand = await fetchCraftCommand(baseCommand.toLowerCase());
    ...
    if (isCraftingCommand) {
        ...
        // new item
        const newItem = await fetchNewCraftItems(craftCommand.material_id);

        // Add crafting logic here
        const { materials, userMaterials } = await fetchCraftItemsRequired(message, craftCommand);
